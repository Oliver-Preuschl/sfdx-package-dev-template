...
(async function () {
  ...
  let subscriptionPackageVersionId = await getSubscriberPackageVersionIdByVersionNumber(
    packageName,
    versionNumber
  );
...
async function getSubscriberPackageVersionIdByVersionNumber(
  packageName,
  versionNumber
) {
  ...
  return packageVersions.find(
    (packageVersion) => packageVersion.Version === versionNumber
